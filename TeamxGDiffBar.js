(function () {
	...
	var margin_bar = {top: 20, right: 30, bottom: 40, left: 50},
	...
	var div = d3.select("#container_xG_Diff_bar").append("div")
    .attr("class", "tooltipbar_team_xg")
    .style("opacity", 0);
	...
	d3.csv("https://raw.githubusercontent.com/jakehughes1882/data/master/UCL_Club_Metrics.csv", type, function(error, data) {	
		...
		data.sort(function(x, y){
			return d3.descending( x.xGDiff, y.xGDiff);
		})
		
		// domains
		xScale.domain(d3.extent(data, function(d) { return d.xGDiff; })).nice();
		...
		// create svg
		var svg = d3.select("#container_xG_Diff_bar")
			.append("svg")
		...
		var thsndFormat = d3.format(",");
			...
			.selectAll(".tick")
			.filter(function(d, i) { return (data[i].xGDiff) < 0; });
	...
	var bars = 	svg.selectAll(".bar")
			...
			.enter()			
			.append("rect")
				.attr("class", function(d) { return "bar bar--" + (d.xGDiff < 0 ? "negative" : "positive"); })
				.attr("rx", 6)
                .attr("ry", 6)
				.attr("y", function(d) { return yScale(d.Team); })
				.attr("height", yScale.bandwidth() - 1)
				//.attr("x",  d => { return width + 300; })
				.attr("x", function(d) { return xScale(Math.min(0, d.xGDiff)); })
			...
			bars.transition()
				...
				.attr("x", function(d) { return xScale(Math.min(0, d.xGDiff)); })
				.attr("width", function(d) { return (Math.abs(xScale(d.xGDiff) - xScale(0))); })	
			...
			bars.on("mouseover", function(d) {
				...
				div.transition()
				 .duration(200)
         .style("opacity", 1.0);
				div.html(d.Team + " scored " + d.Goals + " goals, but had an xG sum of " + d.xGSum + " leaving a difference of " + d.xGDiff)
