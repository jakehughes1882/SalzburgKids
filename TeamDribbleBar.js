(function () {
	...
	var div = d3.select("#container_dribbles_bar").append("div")
    .attr("class", "tooltipbar_team_dribble")
    .style("opacity", 0);
	...
	d3.csv("https://raw.githubusercontent.com/jakehughes1882/data/master/UCL_Club_Metrics.csv", type, function(error, data) {	
		...
		data.sort(function(x, y){
			return d3.descending( x.SuccessfulDribbles, y.SuccessfulDribbles);
		})
		
		// domains
		xScale.domain(d3.extent(data, function(d) { return d.SuccessfulDribbles; })).nice();
		...
		// create svg
		var svg = d3.select("#container_dribbles_bar")
			.append("svg")
	...
	var bars = 	svg.selectAll(".bar")
			...
			bars.transition()
				...
				.attr("x", function(d) { return xScale(Math.min(0, d.SuccessfulDribbles)) + 150; })
				.attr("width", function(d) { return (Math.abs(xScale(d.SuccessfulDribbles) - xScale(0)))*0.8; })	
			...
			bars.on("mouseover", function(d) {
				...
				div.transition()
				 .duration(200)
         .style("opacity", 1.0);
				div.html("Players for " + (d.Team) + " completed " + d.SuccessfulDribbles + " successful dribbles.")
...
function type(d) {
  d.SuccessfulDribbles = +d.SuccessfulDribbles;
  return d;
}
})()
